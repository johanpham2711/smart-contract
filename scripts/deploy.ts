...
async function main() {
  ...
  console.log(
    `Contract owner balance after deployment: ${formatEther(
      balanceAfterDeployment.toString()
    )} ETH`
  );
  console.log(`Deployment gas used: ${receipt.gasUsed} wei`);
  console.log(
    `Deployment gas fee: ${formatEther(deploymentGasFee.toString())} ETH`
  );
